...
impl DataVersion for BodyVersion {}

impl TryFrom<MagicStr> for BodyVersion {
    type Error = M2Error;
...
impl From<BodyVersion> for MagicStr {
    fn from(value: BodyVersion) -> Self {
        match value {
            BodyVersion::V1 => BODY,
            BodyVersion::V2 => BDY2,
            BodyVersion::V3 => BDY3,
...
impl WowHeaderR for BodyVersion {
...
impl WowHeaderW for BodyVersion {
...
#[wow_data(version = BodyVersion)]
pub enum BoneIndex {
    Padding([u8; 2]),

    #[wow_data(read_if = version >= BodyVersion::V3)]
    Some(i16),
}
...
#[wow_data(version = BodyVersion)]
pub enum ShapeIndex {
    #[wow_data(read_if = version <= BodyVersion::V2)]
    ModelBoneIndex(i16),
...
pub enum VE1<T: Default + WowHeaderR + WowHeaderW> {
    ...
    #[wow_data(read_if = version == BodyVersion::V1)]
    Some(T),
}
...
pub enum VGTE4<T: Default + WowHeaderR + WowHeaderW> {
    ...
    #[wow_data(read_if = version >= BodyVersion::V4)]
    Some(T),
}
...
pub struct Body {
    ...
    #[wow_data(versioned)]
    pub unknown0: VE1<[u8; 4]>,
    #[wow_data(versioned)]
    pub unknown1: VGTE2<f32>,
    #[wow_data(versioned)]
    pub drag: VGTE3<f32>,
    #[wow_data(versioned)]
    pub unknown2: VGTE3<f32>,
    #[wow_data(versioned)]
    pub unknown3: VGTE3<f32>,
    #[wow_data(versioned)]
    pub unknown4: VGTE4<[u8; 4]>,
}
