...
use wow_mpq::{Archive, ArchiveBuilder, MutableArchive, AddFileOptions, FormatVersion, ListfileOption};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        let mut archive = Archive::open("/home/danielsreichenbach/Downloads/wow/1.12.1/Data/interface.MPQ")?;
        let data = archive.read_file("Interface\\Glues\\Credits\\TrollBanner4.blp")?;
        test_files.insert("Interface\\Glues\\Credits\\TrollBanner4.blp".to_string(), data);
        println!("✓ Read TrollBanner4.blp: {} bytes", test_files["Interface\\Glues\\Credits\\TrollBanner4.blp"].len());
    }
    
    // File 2: Some other file from the same archive
    {
        let mut archive = Archive::open("/home/danielsreichenbach/Downloads/wow/1.12.1/Data/fonts.MPQ")?;
        let data = archive.read_file("Fonts\\MORPHEUS.TTF")?;
        test_files.insert("Fonts\\MORPHEUS.TTF".to_string(), data);
        println!("✓ Read MORPHEUS.TTF: {} bytes", test_files["Fonts\\MORPHEUS.TTF"].len());
    }
    ...
    if let Ok(output) = compile {
        if output.status.success() {
            let result = Command::new("./test_mod")
                .env("LD_LIBRARY_PATH", "/home/danielsreichenbach/Repos/github.com/ladislav-zezula/StormLib/build")
                .output()?;
