//! Comprehensive test with WoW 4.3.4 files: create V3 archive, verify integrity, add more files, verify again
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🧪 Comprehensive WoW 4.3.4 File Integrity Test");
    println!("==============================================");

    // Find WoW 4.3.4 data
    let cata_data_path = match find_wow_data(WowVersion::Cata) {
        Some(path) => {
            println!("✅ Found Cataclysm data at: {}", path.display());
            path
        }
        None => {
            println!("❌ No WoW 4.3.4 data found!");
            println!();
    ...
    // Find MPQ archives in Cataclysm directory
    let mpq_candidates = [
        // Base archives
        "world.MPQ",
        "world2.MPQ",
        ...
        // Locale archives (in enUS subdirectory)
        "enUS/locale-enUS.MPQ",
        "enUS/speech-enUS.MPQ",
        "enUS/expansion1-locale-enUS.MPQ",
        "enUS/expansion2-locale-enUS.MPQ",
        "enUS/expansion3-locale-enUS.MPQ",
        "enUS/wow-update-enUS-15211.MPQ",
        "enUS/wow-update-enUS-15354.MPQ",
        "enUS/wow-update-enUS-15595.MPQ",
    ];
    let mut cata_archives = Vec::new();

    for &mpq_name in &mpq_candidates {
        let mpq_path = cata_data_path.join(mpq_name);
        if mpq_path.exists() {
            cata_archives.push(mpq_path);
        }
    }

    if cata_archives.is_empty() {
        println!("❌ No MPQ archives found in Cataclysm directory");
        return Ok(());
    }

    println!("📦 Found {} Cataclysm archives:", cata_archives.len());
    for archive_path in &cata_archives {
        println!(
            "  - {}",
    ...
    // Step 2: Create V3 archive with first batch (HET/BET tables)
    println!("\n🔨 Step 2: Creating V3 archive with HET/BET tables, listfile and attributes...");
    let test_archive_path = "test_cataclysm_comprehensive.mpq";

    create_v3_archive(&first_batch_files, test_archive_path)?;
    println!("✅ Created V3 archive: {}", test_archive_path);
    ...
    // Step 5: Add second batch to V3 archive
    println!("\n➕ Step 5: Adding second batch files to existing V3 archive...");
    ...
    println!("\n🔍 Step 6: Verifying ALL files in final V3 archive...");
    ...
    // Step 7: Test V3-specific features
    println!("\n🔬 Step 7: Testing V3-specific features...");
    test_v3_features(test_archive_path)?;
    ...
    if final_verification_ok {
        ...
        println!("✅ wow-mpq can accurately read and write WoW 4.3.4 game files");
        println!("✅ V3 archive format with HET/BET tables working correctly");
        println!("✅ Archive modification (add files) preserves data integrity");
        println!(
            "✅ Cataclysm-specific features (advanced compression, larger archives) handled correctly"
        );
    } else {
...
fn extract_random_files(
    ...
    for archive_path in archives {
        ...
        let regular_files: Vec<_> = file_list
            ...
            .filter(|f| f.size > 0 && f.size < 3 * 1024 * 1024) // Skip empty and very large files (>3MB)
            .filter(|f| !f.name.contains("__MACOSX")) // Skip macOS metadata
            .filter(|f| !f.name.ends_with(".DS_Store")) // Skip macOS files
            .filter(|f| !f.name.to_lowercase().contains("chinese")) // Skip localization files that might be problematic
            .filter(|f| !f.name.to_lowercase().contains("korean"))
            .collect();
        ...
        let sample_size = (target_count - files.len()).min(regular_files.len()).min(5);
...
fn extract_random_files_excluding(
    ...
    for archive_path in archives {
        ...
        let regular_files: Vec<_> = file_list
            ...
            .filter(|f| !f.name.contains("__MACOSX")) // Skip macOS metadata
            .filter(|f| !f.name.ends_with(".DS_Store")) // Skip macOS files
            .filter(|f| !f.name.to_lowercase().contains("chinese")) // Skip localization files
            .filter(|f| !f.name.to_lowercase().contains("korean"))
            .collect();
...
fn create_v3_archive(
    files: &HashMap<String, Vec<u8>>,
    output_path: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    let mut builder = ArchiveBuilder::new()
        .version(FormatVersion::V3) // V3 format with HET/BET tables
...
fn test_v3_features(archive_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    let mut archive = Archive::open(archive_path)?;

    // Test that we can read archive info to verify V3 format
    ...
    // Verify we can list all files (uses HET/BET tables in V3)
    match archive.list() {
        Ok(files) => {
            println!("  📋 File listing successful: {} files", files.len());

            // Test some file lookups to verify HET table functionality
            if files.len() >= 3 {
    ...
    println!("  ✅ V3 features test completed");
    Ok(())
}
