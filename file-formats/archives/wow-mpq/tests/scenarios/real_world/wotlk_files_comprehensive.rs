//! Comprehensive test with WoW 3.3.5a files: create V2 archive, verify integrity, add more files, verify again
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🧪 Comprehensive WoW 3.3.5a File Integrity Test");
    println!("===============================================");

    // Find WoW 3.3.5a data
    let wotlk_data_path = match find_wow_data(WowVersion::Wotlk) {
        Some(path) => {
            println!("✅ Found WotLK data at: {}", path.display());
            path
        }
        None => {
            println!("❌ No WoW 3.3.5a data found!");
            println!();
    ...
    // Find MPQ archives in WotLK directory
    let mpq_candidates = [
        "patch.MPQ",
        "patch.mpq",
        "common.MPQ",
    ...
    let mut wotlk_archives = Vec::new();

    for &mpq_name in &mpq_candidates {
        let mpq_path = wotlk_data_path.join(mpq_name);
        if mpq_path.exists() {
            wotlk_archives.push(mpq_path);
        }
    }

    if wotlk_archives.is_empty() {
        println!("❌ No MPQ archives found in WotLK directory");
        return Ok(());
    }

    println!("📦 Found {} WotLK archives:", wotlk_archives.len());
    for archive_path in &wotlk_archives {
        println!(
            "  - {}",
    ...
    println!("\n🎲 Step 1: Extracting random files from WotLK archives...");
    let mut first_batch_files = HashMap::new();
    let target_first_batch = 10;
    ...
    let test_archive_path = "test_wotlk_comprehensive.mpq";
    ...
    println!("\n➕ Step 5: Adding second batch files to existing V2 archive...");
    ...
    if final_verification_ok {
        ...
        println!("✅ wow-mpq can accurately read and write WoW 3.3.5a game files");
        ...
        println!("✅ WotLK-specific features (LZMA compression, larger files) handled correctly");
    } else {
...
fn extract_random_files(
    ...
    for archive_path in archives {
        ...
        let regular_files: Vec<_> = file_list
            ...
            .filter(|f| f.size > 0 && f.size < 2 * 1024 * 1024) // Skip empty and very large files (>2MB)
        ...
        let sample_size = (target_count - files.len()).min(regular_files.len()).min(4);
